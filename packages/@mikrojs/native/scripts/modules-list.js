#!/usr/bin/env node
/**
 * Print builtin module lists derived from runtime/modules.json for CMake.
 *
 * Usage: node modules-list.js <bytecode|native|features> [--host] [--ble=on|off] [--wifi=on|off]
 *
 ...
 *
 * Output is semicolon-separated (a CMake list) on stdout.
 *
...
if (kind !== 'bytecode' && kind !== 'native' && kind !== 'features') {
  process.stderr.write(
    'usage: modules-list.js <bytecode|native|features> [--host] [--ble=on|off] [--wifi=on|off]\n',
  )
  process.exit(1)
}
...
const names =
  kind === 'bytecode'
    ? included.filter((m) => m.bytecode !== false).map((m) => m.name)
    : kind === 'native'
      ? included.filter((m) => m.native).map((m) => m.native)
      : // A module can need a feature without being compiled under its gate
        // (http/server needs wifi and always compiles), so a feature whose
        // gate is off is dropped here. This relies on each gated feature
        // having the same name as its gate.
        [...new Set(included.filter((m) => m.feature).map((m) => m.feature))].filter(
          (f) => !off.has(f),
        )
