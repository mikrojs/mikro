# mikrojs_generate_bytecode() — CMake function that compiles the core runtime's
# TypeScript modules to QuickJS bytecode headers, as the mikro/* builtins.
#
# Used by the standalone library and the ESP-IDF component.
#
# Usage:
#   mikrojs_generate_bytecode(
#       RUNTIME_DIR <path>         # Directory containing <mod>/<mod>.ts files
#       MODULES <mod1> <mod2>      # Module names to compile ("fs" -> "mikro/fs")
...
function(mikrojs_generate_bytecode)
    cmake_parse_arguments(ARG "" "RUNTIME_DIR;TARGET;WORKING_DIRECTORY" "MODULES" ${ARGN})
    ...
    foreach(mod ${ARG_MODULES})
        ...
        set(_header "${_GEN_DIR}/mikro_${_mod_safe}.h")
        add_custom_command(
            OUTPUT ${_header}
            ...
            COMMAND sh ${_COMPILE_SCRIPT}
                ...
                ${_header}
                "mikro/${mod}"
                "mikro_${_mod_safe}_bytecode"
            DEPENDS ${_BUNDLE_DIR}/bundle.stamp ${_COMPILE_SCRIPT} ${QJSC_EXECUTABLE}
            COMMENT "Compiling bytecode: mikro/${mod}"
        )
        list(APPEND _HEADERS ${_header})
    endforeach()

    # Step 3: Generate builtins table header (includes + lookup table)
    set(_TABLE_HEADER "${_GEN_DIR}/mikro_builtins_table.h")
    set(_TABLE_CONTENT "/* Auto-generated by mikrojs_generate_bytecode() — do not edit */\n")
    foreach(mod ${ARG_MODULES})
        string(REGEX REPLACE "[^a-zA-Z0-9]" "_" _mod_safe "${mod}")
        string(APPEND _TABLE_CONTENT "#include \"mikro_${_mod_safe}.h\"\n")
    endforeach()
    string(APPEND _TABLE_CONTENT "\nstatic const mik_builtin_t mikro_builtins[] = {\n")
    foreach(mod ${ARG_MODULES})
        string(REGEX REPLACE "[^a-zA-Z0-9]" "_" _mod_safe "${mod}")
        string(APPEND _TABLE_CONTENT "    {\"mikro/${mod}\", mikro_${_mod_safe}_bytecode, mikro_${_mod_safe}_bytecode_size},\n")
    endforeach()
