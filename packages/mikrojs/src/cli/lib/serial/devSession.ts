...
export function createDevSession(options: {
  ...
  bytecode: boolean
  watch: boolean
  /** Skip package.json `mikrojs.predeploy` hooks. */
  noHooks?: boolean
  ...
  // runs `mikrojs.predeploy` commands from package.json; on success it feeds
  ...
  function hookStage$(trigger: Trigger): Observable<DevSessionState> {
    // Re-read package.json per trigger so edits to `mikrojs.predeploy` are
