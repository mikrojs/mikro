...
/**
 * Read `mikro.predeploy` from the project's package.json. Returns an
 * empty array if the file, the `mikro` field, or the `predeploy` field
...
export function getPredeployCommands(projectRoot: string): string[] {
  ...
  const pkg = JSON.parse(raw) as {mikro?: {predeploy?: unknown}; mikrojs?: unknown}
  assertNoLegacyMikroConfig(pkg, 'package.json')
  const value = pkg.mikro?.predeploy
