...
describe('snapshotVersion', () => {
  ...
  it('appends a UTC build timestamp as the prerelease', () => {
    expect(snapshotVersion('1.2.3', now)).toBe('1.2.3-snapshot.20260723T003205Z')
  })

  it('extends an existing prerelease with a dot, not a second hyphen', () => {
    expect(snapshotVersion('1.2.3-beta.1', now)).toBe('1.2.3-beta.1.snapshot.20260723T003205Z')
  })

  it('strips build metadata off the base before appending', () => {
    expect(snapshotVersion('1.2.3+foo', now)).toBe('1.2.3-snapshot.20260723T003205Z')
  })

  it('orders a later snapshot above an earlier one', () => {
    const later = new Date(Date.UTC(2026, 6, 23, 0, 32, 6))
    expect(semver.gt(snapshotVersion('1.2.3', later), snapshotVersion('1.2.3', now))).toBe(true)
    // Across a field rollover, where a shorter-string comparison would trip up.
    const nextDay = new Date(Date.UTC(2026, 6, 24, 0, 0, 0))
    expect(semver.gt(snapshotVersion('1.2.3', nextDay), snapshotVersion('1.2.3', later))).toBe(true)
  })

  it('produces valid semver', () => {
    expect(semver.valid(snapshotVersion('1.2.3', now))).not.toBeNull()
    expect(semver.valid(snapshotVersion('1.2.3-beta.1', now))).not.toBeNull()
  })
})
