...
```ts
function I2s(port: number, options: I2sStdTxRxOptions): Result<I2s & I2sTx & I2sRx, I2sError>
function I2s(port: number, options: I2sStdTxOptions): Result<I2s & I2sTx, I2sError>
function I2s(port: number, options: I2sStdRxOptions): Result<I2s & I2sRx, I2sError>
function I2s(port: number, options: I2sPdmRxOptions): Result<I2s & I2sRx, I2sError>
```

Claims the pins, allocates the I2S channels, starts the DMA and returns a [`Result`](/api/result) with the handle. The clock pins and `dout` must be able to drive a signal, or you get `InvalidGpio`. If another handle, a peripheral or the console holds a pin, you get `GpioInUse`. A port, sample rate or DMA size out of range returns `InvalidParam`.
...
PDM RX is available on the classic ESP32 and ESP32-S3, but not on every C/H-series target. On a chip without PDM RX, `I2s()` returns a `ChannelInitFailed` error.
:::
...
Blocking bulk read: returns exactly `frames` mono samples, already converted to 16-bit PCM in C, as one packed `Int16Array`. Requires a 32-bit mono channel (`bitsPerSample: 32, channels: 'mono'`) and a `din` pin. A `frames` value that is not a whole number from 1 to 16777216 returns `InvalidParam`.
...
| `InvalidParam`      | `message`          | An option or `frames` is out of range, or a sample width mismatch |
